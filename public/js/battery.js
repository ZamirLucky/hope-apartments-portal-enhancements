document.addEventListener("DOMContentLoaded", () => {
    ...
    try{
        ...
        const getNearbyDevices = document.getElementsByClassName("getNearbyDevices");
        ...
        function appendBatteryRow(battery) {
            try{
                ...
                nearByButton.className = "btn btn-primary btn-sm getNearbyDevices";
                nearByButton.dataset.smartlockId = battery.smartlockId || "";
                nearByButton.dataset.name = battery.name || "";
                nearByButton.setAttribute("data-bs-toggle", "modal");
                nearByButton.setAttribute("data-bs-target", "#staticBackdrop");
                nearByButton.textContent = "Nearby";
                actionsCell.appendChild(nearByButton);
                row.appendChild(actionsCell);

                tableBody.appendChild(row);

            } catch (error) {
                console.error("Error appending battery row:", error, battery);
            }
        }
        ...
        /*
        * Event listeners for elements click
        */
        ...
        if (sortButtonByPerc){
            ...
            console.warn("Sort button not found in DOM");
        }

        // event delegation for dynamically created "Nearby" buttons
        document.body.addEventListener("click", function(event) {
            if (event.target && event.target.classList.contains("getNearbyDevices")) {
                const smartlockId = event.target.dataset.smartlockId;
                const name = event.target.dataset.name || "Unknown";
                console.log(`Nearby button clicked for Smartlock ID: ${smartlockId}, Name: ${name}`);
        ...
        console.error('battery.js init crash:', error);
    }
